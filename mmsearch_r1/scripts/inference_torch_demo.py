...
def parse_titles_from_search_results(tool_returned_str):
    """
    从图像搜索结果字符串中解析网页标题。
    
    Args:
        tool_returned_str: call_image_search 返回的字符串，格式包含 "title: xxx" 行
        
    Returns:
        List[str]: 解析出的标题列表，按顺序排列
    """
    titles = []
    # 匹配模式：数字. 开头的行，后面跟着 title: xxx
    # 支持多行匹配，处理 image 标记可能占多行的情况
    pattern = r'(\d+)\.\s+(?:image:.*?\n)?title:\s*(.+?)(?:\n|$)'
    matches = re.findall(pattern, tool_returned_str, re.MULTILINE)
    
    # 按位置序号排序并提取标题
    if matches:
        # 将匹配结果转换为 (position, title) 元组列表
        title_dict = {int(pos): title.strip() for pos, title in matches}
        # 按位置顺序提取标题
        titles = [title_dict[pos] for pos in sorted(title_dict.keys())]
...
def main():
    ...
    if re.search(r'<search><img></search>$', assistant_first_reply):
        ...
        # Parse real webpage titles from tool_returned_str
        img_tool_returned_web_title_list = parse_titles_from_search_results(tool_returned_str)
        
        # 确保标题数量与图像数量匹配（如果解析失败，使用备用标题）
        if len(img_tool_returned_web_title_list) != len(tool_returned_images):
            print(f"[Warning] Parsed {len(img_tool_returned_web_title_list)} titles but got {len(tool_returned_images)} images. Using fallback titles.")
            # 如果解析的标题数量不匹配，使用序号作为备用
            img_tool_returned_web_title_list = [
                img_tool_returned_web_title_list[i] if i < len(img_tool_returned_web_title_list) 
                else f"Search Result {i+1}" 
